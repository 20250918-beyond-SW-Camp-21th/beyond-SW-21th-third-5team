  ...
  <nav class="w-full h-[72px] bg-white/80 backdrop-blur-sm border-b border-[#E6EEF9] sticky top-0 z-50">
    <div class="max-w-[1440px] mx-auto px-20 h-full flex items-center justify-between">
      <div class="flex-shrink-0">
        <h1 class="text-[20px] font-semibold text-[#1F2A37]">
          자란날씨 <span class="text-[12px] text-[#6B7280]">({{ props.isAuthed ? "LOGGED" : "GUEST" }})</span>
        </h1>
...
const props = defineProps<{ activeTab?: Tab; simple?: boolean; isAuthed?: boolean }>();
const emit = defineEmits<{ "tab-change": [tab: PageKey] }>();

const activeTab = computed(() => props.activeTab ?? "오늘의 날씨");
const authButtonLabel = computed<AuthTab>(() => (props.isAuthed ? "로그아웃" : "로그인"));

function onAuthClick() {
  if (props.isAuthed) {
    emit("tab-change", "로그아웃");
    return;
  }
